...
// IsClaimVolumeLost determines if a PVC's volume is on a node that
// does not exist anymore.
// To decide that, it examines the Volume's NodeSelector
// and checks that each referenced node exists.
func IsClaimVolumeLost(pvc *corev1.PersistentVolumeClaim, kubeclient kubernetes.Interface) (bool, error) {
