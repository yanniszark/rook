	...
	for _, pod := range pods {
		if !podutil.IsPodReady(pod) {
			...
			for _, vol := range pod.Spec.Volumes {
				...
				isVolumeLost, err := util.IsClaimVolumeLost(pvc, cc.kubeClient)
				if err != nil {
				...
				if isVolumeLost {
					...
					// Delete Pod
					err = cc.kubeClient.CoreV1().Pods(pod.Namespace).
						Delete(pod.Name, &metav1.DeleteOptions{})
					if err != nil {
						return fmt.Errorf("Error deleting Pod %s in namespace %s: %s", pod.Name, pod.Namespace, err)
					}
					cc.recorder.Event(
						c,
						corev1.EventTypeWarning,
